...
| 2 | IndexRebuilt | ACT-002 | `docir reindex [--changed]` | maintenance_service.py:184 (`reindex`) → index_rebuilder.py |
| 3 | StructureChecked | ACT-003 | `docir check [--strict]` | maintenance_service.py:205 (`check`) |
| 4 | DuplicateIdDetected | system | file scan, not index | maintenance_service.py:580 (`_find_duplicate_ids`) |
...
| 7 | AdvisoryLinted | ACT-002 | `docir lint --deep` | maintenance_service.py:616 (`lint_deep`) |
| 8 | EmbeddingsRebuilt | ACT-002 | `docir embed --flush`, or any full `docir reindex` | maintenance_service.py:196 (`flush_embeddings`) |
| 9 | UnmatchedCodeFlagged | system | a governed `code:` glob matches nothing the repository tracks | checks/verification_rules.py:89 (`_find_unmatched_code`), maintenance_service.py:446 (`_resolve_code`) |
