...
The frontmatter every document carries, grouped by what it answers. Each
field names the command that sets it; none is written by hand.
...
| Field | Required | Set by | Description |
|---|---|---|---|
| `type` | yes | `docir add` (fixed at creation) | Document type (`decision`, `issue`, `architecture`, ...); selects the grammar that applies. That grammar is **not** only `docs-schema.yaml`: the frozen core and the named profiles are merged in from the installed package on every command, so an upgrade can change a type's rules with nothing in `git diff` — see `arch-ad342aae8293`, "Schema drift and the index build stamp" |
...
`arch-0368cc754c15`, "Archiving vs. deletion" below.
...
### Referential integrity (Tier 0)

every key in a document's `tags`
