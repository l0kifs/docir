...
- retrieval
- embeddings
title: FLOW-002 — Retrieve relevant context (the read path)
type: architecture
updated: '2026-07-30'
---

# FLOW-002 — Retrieve relevant context (the read path)
...
- **H2 — visibility rules disagree between step 6 and step 7.** Step 7 checks `archived` but
  ...
  `--include-resolved`, while `search`/`query` correctly hide it. → `GAP-004`.
- **H3 — no relevance floor.** Every active document receives a semantic rank, so the fused
  ...
  cannot distinguish "here is what matters" from "nothing matches". → `GAP-017`.
- **H4 — the `score` is not comparable across queries.** RRF output depends only on rank
  ...
  Same root cause as H3, separate consequence for anyone thresholding on it. → `GAP-017`.
...
- **H6 — semantic ranking loads every active vector into memory on every call**
  ...
  pagination, no test at scale. Recorded as a limit-of-validity question, not a defect. → `GAP-039`.
- **H7 — graph expansion follows outgoing edges only.** A decision that supersedes an ADR is
  ...
  reader needs ("has this been replaced?"). → `GAP-019`.
...
GAP-004, GAP-005, GAP-017, GAP-018, GAP-019, GAP-039
