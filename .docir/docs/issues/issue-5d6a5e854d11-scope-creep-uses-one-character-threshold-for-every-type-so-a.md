...
- schema
- material
title: '`scope-creep` uses one character threshold for every type, so a register is
  always too long'
type: issue
updated: '2026-08-05'
---
...
document types added to hold long reference material. Together with issue-08437ba704ff it means all 21
findings `lint --deep` produces against the product's own corpus are unactionable.

The parallel is `orphan` under `--strict` (issue-9cb85759076d): a warning that fires on the default
