...
**Flow:** arch-3e305bc76ff0 · **Step:** between the file write and the index commit
**Question:** issue-88dd653b9f39 · **Frequency:** unknown; requires a crash or kill mid-write
...
MITIGATED 2026-07-26 by the same change as issue-b7ddde3ce860: a crash between the file write and the commit still leaves an unindexed file, but the next `add` no longer silently overwrites it — the create is refused with DuplicateDocumentIdError pointing at `docir reindex`. The underlying ordering (file written before commit) is unchanged and still undocumented.
