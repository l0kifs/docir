...
- retrieval
- material
title: GAP-017 — There is no relevance floor and the emitted `score` carries no absolute
  meaning, so…
type: issue
updated: '2026-07-30'
---

# GAP-017 — There is no relevance floor and the emitted `score` carries no absolute meaning, so…

**Class:** misleading · **Severity:** material · **Confidence:** observed
**Flow:** FLOW-002 · **Step:** interpreting context results
**Question:** Q-009 · **Frequency:** every context query with no genuinely relevant document
...
An agent cannot distinguish relevant context from noise, so it either always trusts the results (and is misled on unrelated tasks) or never does. Compounds GAP-005: the response is both unbounded in size and unfiltered in quality.
