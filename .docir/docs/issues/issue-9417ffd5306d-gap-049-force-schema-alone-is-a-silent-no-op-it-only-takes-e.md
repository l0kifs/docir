...
- cli
- cosmetic
title: GAP-049 — `--force-schema` alone is a silent no-op; it only takes effect together
  with `--force`
type: issue
updated: '2026-07-30'
---

# GAP-049 — `--force-schema` alone is a silent no-op; it only takes effect together with `--force`

**Class:** unstated · **Severity:** cosmetic · **Confidence:** observed
**Flow:** FLOW-004 · **Step:** docir init --force-schema without --force
...
Treat `--force-schema` as implying `--force` for the schema, or say why it did nothing. Introduced with the GAP-026 fix.
