...
- daemon
- blocking
title: GAP-051 — the client's connect timeout also bounded the daemon's reply, so
  any command slower than 5s failed
type: issue
updated: '2026-07-30'
---

# GAP-051 — the client's connect timeout also bounded the daemon's reply, so any command slower than 5s failed

**Class:** incorrect · **Severity:** blocking · **Confidence:** observed
**Flow:** FLOW-003 (integrity) · **Step:** any command dispatched over the daemon socket
