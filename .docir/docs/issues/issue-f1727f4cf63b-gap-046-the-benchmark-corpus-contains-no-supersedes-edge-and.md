...
- embeddings
- material
title: GAP-046 — The benchmark corpus contains no `supersedes` edge and no document
  in an inactive…
type: issue
updated: '2026-07-30'
---

# GAP-046 — The benchmark corpus contains no `supersedes` edge and no document in an inactive…

**Class:** missing · **Severity:** material · **Confidence:** observed
**Flow:** FLOW-002 · **Step:** measuring a retrieval change
...
OBSERVED 2026-07-27. The GAP-004 + GAP-019 fixes changed retrieval semantics on both the visibility and the traversal path, and every benchmark number was byte-identical either side of them (recall@5 0.96 fastembed / 0.88 deterministic). All 20 documents use bare `related:` ids, which resolve to `relates_to`, and all statuses are type defaults.

## Impact

A harness that cannot move is read as "no regression" when it means "not measured". The same blindness applies to any future change to expansion, visibility or relation kinds — the parts of `context` that distinguish it from `search`, and therefore the parts most worth a regression signal. GAP-001 was closed by building this harness; this is the boundary of what it closed.
