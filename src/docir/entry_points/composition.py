...
#: The store's committed settings, written by ``init`` and topped up by
#: ``self upgrade``. It is the store's *answer*, not docir's default — which is
#: why the value is written out rather than left implicit, and why a store that
#: sets it to ``false`` keeps that through both commands.
#:
#: Creating a store is the act that opts into the one network call docir makes
#: (adr-a555ee6bc484 keeps it from happening unasked). Recording the answer here
#: rather than in each person's shell is what makes it a decision the team takes
#: once: the file is committed, so a teammate's clone and their agent inherit it,
#: and ``DOCIR_UPDATE_CHECK=0`` is how one of them opts back out.
_STORE_CONFIG = """\
# docir store settings — committed, and shared by everyone who clones this repo.
# Per-machine choices do not belong here; they are environment variables.

# Tell this store's readers when a newer docir has been published. The daemon
# then asks PyPI at most once a day (docir's only network call) and each command
# prints at most one line about it. `DOCIR_UPDATE_CHECK=0` overrides it for one
# shell; a CI run never checks.
update_check: true
"""


def refresh_store_config(home: Path) -> tuple[str, ...]:
    """Add the settings this build writes that ``home``'s config lacks.

    The same contract as :func:`refresh_store_gitignore`, for the same reason: a
    store created before a setting existed should gain it on the upgrade its
    owner runs anyway, and a store that has answered the question already keeps
    its answer. So this **appends and never rewrites** — a key already present,
    whatever its value, is a decision somebody took.
    """
    path = home / STORE_CONFIG_FILENAME
    existing = path.read_text(encoding="utf-8") if path.exists() else ""
    if not existing.strip():
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(_STORE_CONFIG, encoding="utf-8")
        return _config_keys(_STORE_CONFIG)
    declared = set(_config_keys(existing))
    missing = tuple(key for key in _config_keys(_STORE_CONFIG) if key not in declared)
    if not missing:
        return ()
    block = "".join(f"{key}: true\n" for key in missing)
    path.write_text(f"{existing.rstrip()}\n\n{block}", encoding="utf-8")
    return missing


def _config_keys(text: str) -> tuple[str, ...]:
    """The top-level keys a store config declares.

    Read as text rather than as YAML on purpose: this decides what to *append*
    to a file a person may have commented, and a parse-and-dump round trip is how
    those comments get deleted by a command that was asked to add one line.
    """
    keys = []
...
class InitResult:
    ...
    #: Whether this run wrote the store's ``config.yaml`` (or added a key to it).
...
def initialize_store(
    ...
    *,
    ...
    # Not regenerated by ``force``, unlike the gitignore beside it: this file
    # holds an answer somebody gave, and the gitignore holds a constant this
    # module generates. ``--force`` exists to repair the generated files.
    config_written = bool(refresh_store_config(settings.home))
...
class UpgradeResult:
    ...
    #: Settings this build writes that the store's ``config.yaml`` was missing.
    config_added: tuple[str, ...] = ()
...
def upgrade_store(
    ...
    * ``check`` — **last**, so its findings describe the state the upgrade left
    ...
    config_added = refresh_store_config(store_home)
    findings = run("check", {})
